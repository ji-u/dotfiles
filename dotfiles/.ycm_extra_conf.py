...
def FLagsFromCompilationCommands(root):
    try:
        compilation_db_path = FindNearest(root, 'compile_commands.json', 'build')
        with open(compilation_db_path) as f:
            commands = json.load(f)
        flags = []
        for command in commands:
            flag_iter = iter(command['command'].split(' ')[1:])
...
def FlagsForFile(filename):
    ...
    compilation_db_flags = FLagsFromCompilationCommands(root)
